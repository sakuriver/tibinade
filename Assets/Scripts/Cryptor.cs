    ...
    public static byte[] Decrypt(byte[] encryptedData, string key, string iv)
    {
        ...
        using (AesManaged aes = new AesManaged())
        {
            ...
            using (MemoryStream encryptedStream = new MemoryStream(encryptedData))
            {
                ...
                {
                    using (CryptoStream cryptoStream = new CryptoStream(encryptedStream, decryptor, CryptoStreamMode.Write))
                    {
#if STREAM_COPY
                        cryptoStream.CopyTo(decompressedStream);
#else
                        var buffer = new byte[1024];

                        for (; ; )
                        {
                            var len = cryptoStream.Read(buffer, 0, buffer.Length);
